...
所有 `.png` 文件都将被发送到输出目录，并且其路径将被注入到 bundle 中，除此之外，你可以为它们自定义 [`outputPath`](/configuration/module/#rulegeneratoroutputpath) 和 [`publicPath`](/configuration/module/#rulegeneratorpublichpath) 属性。
...
自 webpack 5.38.0 起，[Data URLs](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Basics_of_HTTP/Data_URIs) 也支持在 `new URL()` 中使用了：
