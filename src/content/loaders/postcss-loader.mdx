...
|    [`postcssOptions`](#options)     | `{Object\|Function}` | `Postcss.process的默认值` | 设置 `PostCSS` 选项与插件                 |
|     [`sourceMap`](#source-map)      |     `{Boolean}`      |    `compiler.devtool`     | 开启 / 关闭 source map 的生成             |
...
如果你需要 source map，请使用 [`sourcemap`](#source-map) 选项。
...
> :warning: 如果你正在使用 Babel 且为了程序正常工作，你需要进行下面的步骤：
